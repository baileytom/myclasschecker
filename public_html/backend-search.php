    ...
    if($result = mysqli_query($link, $sql)){
        if(mysqli_num_rows($result) > 0){
            while($row = mysqli_fetch_array($result)){
                echo '<li onclick="" class="list-group-item">';
                #echo "<p>" . $row['crn'] . " " . $row['semester'] . " " . $row['year'] . " " . $row['name'] . " " . $row['day'] . " " .$row['time'] . "</p>";
                $_result = "%s</br>%s</br>%s | %s | %s</br> %s, %s</br>";
                echo "".sprintf($_result, $row['name'], $row['professor'], $row['location'], $row['day'], $row['time'], $row['crn'], $row['semester']." ".$row['year'])."";
                echo '</li>';
            }
            // Close result set
